...
class DialogflowFulfillment {
    ...
    /**
     * setGenericCards sets generic cards for FACEBOOK_MESSENGER platforms
     * @param {Array} cardData The object containing required data for cards
     * @example
     *  [{
     *      "formattedText": "Card Description (required only for ACTIONS_ON_GOOGLE)",
     *      "title": "Card Title (optional for ACTIONS_ON_GOOGLE, required for FACEBOOK_MESSENGER)",
     ...
     *  }
     */
    setGenericCards(cardData) {
        let basicCardsResponse = responseBuilder.buildGenericCards(this._config.platformsEnabled, cardData);
        basicCardsResponse.forEach(element => {
            this._response.fulfillmentMessages.push(element);
        });
    }
